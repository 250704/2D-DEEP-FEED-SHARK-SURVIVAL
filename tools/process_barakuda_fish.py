...
def main() -> None:
    ...
    src = root / "assets" / "barakuda" / "ChatGPT Image 20 Mei 2026, 09.48.07.png"
    out_dir = root / "assets" / "fish" / "barakuda"
    ...
    comps_by_row = sorted(comps, key=lambda c: c.cy)
    row1 = sorted(comps_by_row[:4], key=lambda c: c.cx)
    row2 = sorted(comps_by_row[4:], key=lambda c: c.cx)
    ordered = row1 + row2
    ...
    alpha_soft = np.clip((chroma - 2) * 22, 0, 255).astype(np.uint8)
    pad = 18
    crops = []
    for c in ordered:
    ...
    print(
        "METHOD: checkerboard removed with chroma-derived alpha, 8 fish extracted using connected-components, padded crop to preserve fins/tail, centered on consistent transparent canvases."
    )
